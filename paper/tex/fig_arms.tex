...
%
% Geometry note: TMLR's \textwidth is 6.5in = 16.5cm. Everything is laid out to
% sit inside that, with the arm descriptions in a fixed-width left column rather
% than as free text to the right of the circles -- free text there is what made
% an earlier version overflow the margin, which is a silent error that only
% shows up in the PDF.
\begin{figure}[t]
\centering
\begin{tikzpicture}[
  ...
  desc/.style   = {font=\scriptsize, text=black!60, text width=40mm, align=right},
]
...
\def\ux{0.52}          % unit pitch
\def\xend{5.72}        % 11 * \ux

% ---- the score ordering, shared by all three panels ---------------------------
\node[sub, anchor=west] at (-0.28,2.34)
  {one layer, at one recycling event, with its units ordered by Sokar score $s_i$};

\draw[decorate, decoration={brace, amplitude=3pt}, black!55]
  (-0.28,1.46) -- (4*\ux+0.28,1.46)
  node[midway, above, sub, yshift=0.6mm] {$\tau$-dormant set, size $k$};

\foreach \i in {0,...,11}{
  \pgfmathsetmacro\xx{\i*\ux}
  \ifnum\i<3 \node[dead]  at (\xx,1.16) {}; \else
  \ifnum\i<5 \node[quiet] at (\xx,1.16) {}; \else
             \node[alive] at (\xx,1.16) {}; \fi\fi
}
\draw[->, black!45, line width=0.5pt] (-0.32,0.80) -- (\xend+0.32,0.80);
\node[sub, anchor=west] at (-0.32,0.55) {$s_i$ low};
\node[sub, anchor=east] at (\xend+0.32,0.55) {$s_i$ high};

\node[desc, anchor=east] at (-0.60,1.05)
  {$k$ is read off the data at \emph{every} event in \emph{every} layer --- not a
   fixed fraction, not a schedule};

\draw[black!25, line width=0.4pt] (-4.75,0.26) -- (\xend+0.35,0.26);
...
\foreach \arm/\name/\col/\sel/\note in {%
  ...
  \pgfmathsetmacro\yy{\rowy - \arm*0.95}
  \node[desc, anchor=east, text=\col] at (-0.60,\yy+0.235)
    {\textbf{\footnotesize \name}};
  \node[desc, anchor=east] at (-0.60,\yy-0.185) {\note};
  \foreach \i in {0,...,11}{
    \pgfmathsetmacro\xx{\i*\ux}
...
% ---- legend ------------------------------------------------------------------
\def\ly{-3.10}
\draw[black!25, line width=0.4pt] (-4.75,\ly+0.46) -- (\xend+0.35,\ly+0.46);
\node[dead]  at (-4.65,\ly) {}; \node[sub, anchor=west] at (-4.48,\ly) {exactly zero};
\node[quiet] at (-2.55,\ly) {}; \node[sub, anchor=west] at (-2.38,\ly) {quiet, but firing};
\node[alive] at (-0.10,\ly) {}; \node[sub, anchor=west] at ( 0.07,\ly) {ordinary};
\node[unit, pick=black!55] at (1.55,\ly) {};
\node[sub, anchor=west] at (1.72,\ly) {coloured outline: recycled at this event};
